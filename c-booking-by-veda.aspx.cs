...
public partial class c_booking_by_veda : System.Web.UI.Page
{
...
FROM            bookinginfo INNER JOIN
                         ...
                         customer_signup ON bookinginfo.email_id = customer_signup.email_id where partner_lead_booking.status='Booked' and bookinginfo.pick_a_Date=convert(date, getdate()) and customer_signup.email_id='" + Session["cus_email_id"].ToString() + "' order by bookinginfo.Booking_Id desc ");
...
FROM            bookinginfo INNER JOIN
                         Partner_Lead_Booking INNER JOIN
                         PartnerInfo ON Partner_Lead_Booking.Partner_MobileNo = PartnerInfo.mobile ON bookinginfo.booking_no = Partner_Lead_Booking.Booking_No INNER JOIN
                         customer_signup ON bookinginfo.email_id = customer_signup.email_id where partner_lead_booking.status='Booked' and bookinginfo.pick_a_Date between '" + txtfromdate1.Text + "' and '" + txttodate1.Text + "' and customer_signup.email_id='" + Session["cus_email_id"].ToString() + "' order by bookinginfo.Booking_Id desc ");
            if (sds.Tables[0].Rows.Count > 0)
            {
